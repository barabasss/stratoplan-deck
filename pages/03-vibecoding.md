...
<!--
Сценарии, когда вайбкодинг реально применяется: 1) быстрый прототип фичи до постановки
задачи команде; 2) проверка гипотезы живым приложением (показать клиенту/инвестору/СД
за день-два, а не презентацией); 3) персональные инструменты под свой процесс.
Внизу мелким — мои личные примеры: приложение для инвестиций, тренажёр английских слов,
парсер телеграм-каналов с топом постов.
-->
...
<!--
Реальный кейс коллеги: ресурсное планирование в матричной структуре (РП заявляют
потребность, РМ дают людей). Жило в одном облачном Excel, который параллельно правят
~30 человек. Конкретика боли (для голоса): 39 человек перегружены, 5 — за 200%;
49 строк рассинхрона должность↔ФИО; 143 свободных варианта названия одной роли;
одна вакансия задвоена 14 раз; ~732 строки назначений. Это не «корпоративный портал»,
а как раз та задача, до которой не доходят руки, — идеальный повод для вайбкодинга.
-->
...
<!--
Три реальных скрина того самого Excel — внахлёст, как бумажные фото на столе. Наведи
на любой — он крупно показывается по центру (увёл курсор — спрятался). Так в зале реально
видно «портянку». Дальше — что стало.
-->
...
<!--
Левая колонка — боли Excel, правая — что реально умеет инструмент (сверено с кодом:
FastAPI + PostgreSQL, развёрнут в проде, CI/CD). Ключевой сдвиг — должность/грейд
принадлежат человеку, а не назначению → рассинхрон должность↔ФИО невозможен by design;
и разделение «РП заявляет потребность / РМ назначает человека» вместо переписывания
одной ячейки. «Два дня» — это был старт на первые роли; дальше инструмент дорос.
-->
...
<!--
Вывод из кейса: вайбкодингом реально собрать с нуля рабочий сервис под свою задачу.
Для скептиков: «а нельзя взять готовое?» — иногда да, но не всегда есть готовое под твой
процесс; по моему опыту (живой пример resplan) проблемы часто реальны и их надо решать.
Трейдофы: не про массовый продакшен (нагрузка, SLA, масштаб) — но точно закрывает локальные
боли, а в лучшем случае дорастает до инструмента на команду/всю компанию.
-->
...
<!--
Главный паттерн вайбкодинга: не «сделай и посмотрим», а сначала договориться о плане.
Правка плана стоит копейки, правка готового кода – дорого. Дальше – зачем на сложных
задачах нужны фреймворки.
-->
...
<!--
Суть фреймворка: заставляет идти по формату и не продалбываться — меньше шансов
пропустить важный шаг. Дальше — какие фреймворки бывают.
-->
...
<!--
ПОМЕТКА: сначала упомянуть, что классический режим планирования (plan mode) есть во всех
существующих агентах (Claude Code, Codex, Cursor…) — это база; фреймворки идут дальше.
Беглый обзор. Три фреймворка-скилла работают внутри агента (spec-kit, gstack, Superpowers).
ralphex стоит особняком — это утилита поверх агента: запускает агента в цикле по плану,
сама не пишет код. Рекомендую Superpowers: он на скиллах, цельный путь.
-->
...
<!--
Три стадии Superpowers: обсуждение (brainstorming) фиксирует спеку, writing-plans делает
план, дальше реализация по плану. Внутренние имена знать не обязательно — важны три стадии,
и при отклонении можно вернуть агента к плану.
-->
...
<!--
Переходы между шагами — три механизма вместе: 1) триггер по description (агент сам активирует
скилл, когда ситуация подходит); 2) явная эстафета — скилл в конце называет следующий
REQUIRED SUB-SKILL; 3) гейты пользователя — на ключевых точках твоё подтверждение, на блокере
останавливается. Пример: brainstorming в конце сам зовёт writing-plans.
-->
...
<!--
Демо 6, шаг 0 — установка. Идея: дай агенту ссылку на репозиторий Superpowers, он подскажет
команду под твой харнесс. Для Claude Code — плагин из официального маркетплейса. После
установки скиллы авто-срабатывают с первого сообщения. Дальше — проходим флоу на фиче resplan
(экспорт в Excel). Важно: это искусственное демо-объяснение, не запись живого прогона.
-->
...
<!--
Шаг 1 (brainstorming). Агент вопросами вытаскивает требования. РЕЗУЛЬТАТ шага по самому
скиллу: написанный design-doc (спека) — сохраняется в docs/superpowers/specs/ДАТА-...-design.md
и коммитится в git; ты её ревьюишь перед планом. Справа — пример спеки по нашей фиче.
-->
...
<!--
Шаг 3 (ветку агент завёл сам). Агент пишет план задачами по 2–5 минут с путями файлов
и шагами TDD. Ты его читаешь и одобряешь — поправить план дёшево, поправить код дорого.
-->
...
<!--
Шаг 4. По плану: падающий тест, видит RED, минимальный код, GREEN, ревью, коммит.
Делает сам, тебя не дёргает — остановится только если упрётся.
-->
...
<!--
Шаг 5. Сначала прогоняет тесты (должны пройти), потом предлагает 4 варианта. Решаешь ты.
Для resplan main = прод, поэтому PR. Итог демо: ты участвуешь на 3 точках, остальное —
агент по флоу.
-->
...
<!--
Итог демо: так, шаг за шагом, и реализовали фичу через Superpowers. Карта пяти шагов
с метками — где нужен ты (обсудить, ревью плана, финал), где агент сам (ветка, реализация).
-->
